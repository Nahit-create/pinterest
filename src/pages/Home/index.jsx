...
const Home = () => {
  ...
  useEffect(() => {
    setNotFound(false);
    setLoading(true);
    axios.get(`photos?per_page=28&page=${pageRandom}`).then((pin) => {
  ...
  useEffect(() => {
    ...
    if (search !== '') {
      axios
        .get(`search/photos?query=${search}&per_page=28&page=${pageSearch}`)
        .then((pin) => {
          if (pin.data.total === 0) {
            setNotFound(true);
          }
  ...
  return (
    <div className='home'>
      <div className='container'>
        {loading ? (
          <Loader text="We're adding new Trends to your home feed!" />
        ) : null}
        {notFound ? <Loader text='Ooops! Requested Resource  Not Found...' action /> : null}
